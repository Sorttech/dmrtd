...
void main() {
  ...
  test('DESedeCipher Encryption/Decryption with no padding', () {
    ...
    final tvEifd =
        "72C29C2371CC9BDB65B779B8E8D37B29ECC154AA56A8799FAE2F498F76ED92F2"
            .parseHex();
    final Eifd =
        DESedeCipher(key: tvKenc, iv: zeroIV).encrypt(tvS, padData: false);
    expect(Eifd, tvEifd);

    final dS =
        DESedeCipher(key: tvKenc, iv: zeroIV).decrypt(Eifd, paddedData: false);
    expect(dS, tvS);
    ...
    final tvEic =
        "46B9342A41396CD7386BF5803104D7CEDC122B9132139BAF2EEDC94EE178534F"
            .parseHex();
    final Eic =
        DESedeEncrypt(key: tvKenc, iv: zeroIV, data: tvR, padData: false);
    expect(Eic, tvEic);

    final dR =
        DESedeDecrypt(key: tvKenc, iv: zeroIV, edata: Eic, paddedData: false);
    expect(dR, tvR);
  });
  ...
  test('DESedeCipher Encryption/Decryption with padding', () {
    ...
    var edata =
        DESedeCipher(key: tvKSenc, iv: zeroIV).encrypt(tvData, padData: true);
    expect(edata, tvEData);

    var data =
        DESedeCipher(key: tvKSenc, iv: zeroIV).decrypt(edata, paddedData: true);
    expect(data, tvData);
    ...
    edata = DESedeCipher(key: tvKSenc, iv: zeroIV)
        .encrypt(tvData); // should be padded by default
    expect(edata, tvEData);

    data = DESedeCipher(key: tvKSenc, iv: zeroIV)
        .decrypt(edata); // should be unpadded by default
    expect(data, tvData);
    ...
    edata = DESedeEncrypt(
        key: tvKSenc, iv: zeroIV, data: tvData); // should be padded by default
    expect(edata, tvEData);

    data = DESedeDecrypt(
        key: tvKSenc,
        iv: zeroIV,
        edata: edata); // should be unpadded by default
    expect(data, tvData);
  });
}
