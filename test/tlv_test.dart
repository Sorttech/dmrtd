...
void main() {
  test('TLV encoding test', () {
    ...
    expect(TLV.encode(0x87, "016375432908C044F6".parseHex()),
        "8709016375432908C044F6".parseHex());
    expect(TLV.encode(0x8E, "BF8B92D635FF24F8".parseHex()),
        "8E08BF8B92D635FF24F8".parseHex());
    expect(TLV.encode(0x8E, "ED6705417E96BA55".parseHex()),
        "8E08ED6705417E96BA55".parseHex());
    expect(TLV.encode(0x8E, "2EA28A70F3C7B535".parseHex()),
        "8E082EA28A70F3C7B535".parseHex());
    ...
    expect(
        ...
        throwsTLVError(
            message:
                "Can't encode negative or greater than 16 777 215 length"));
    expect(
        () => TLV.encodeLength(0x10000000),
        throwsTLVError(
            message:
                "Can't encode negative or greater than 16 777 215 length"));
  });
  ...
  test('TLV decoding test', () {
    ...
    expect(do87.tag.value, 0x87);
    expect(do87.value,
        "01FB9235F4E4037F2327DCC8964F1F9B8C30F42C8E2FFF224A".parseHex());
    expect(do99.tag.value, 0x99);
    expect(do99.value, "9000".parseHex());
    expect(do8E.tag.value, 0x8E);
    ...
    // Fuzz testing
    expect(() => TLV.decode(Uint8List(0)),
        throwsTLVError(message: "Can't decode empty encodedTag"));
    expect(() => TLV.decode("00".parseHex()),
        throwsTLVError(message: "Can't decode empty encodedLength"));
    expect(() => TLV.decode("1F".parseHex()),
        throwsTLVError(message: "Invalid encoded tag"));
    expect(() => TLV.decode("1F80".parseHex()),
        throwsTLVError(message: "Invalid encoded tag"));

    expect(
        () => TLV.decodeLength("".parseHex()),
        throwsTLVError(
            message: "Can't decode empty encodedLength")); // No byte data
    expect(() => TLV.decodeLength("82".parseHex()),
        throwsTLVError(message: "Invalid encoded length")); // Missing 2 bytes
    expect(
        () => TLV.decodeLength("8410000000".parseHex()),
        throwsTLVError(
            message: "Encoded length is too big")); // Encoded length too big
    expect(() => TLV.decode("0082".parseHex()),
        throwsTLVError(message: "Invalid encoded length")); // Missing 2 bytes
    expect(
