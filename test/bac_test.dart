...
void main() {
  test('BAC key seed test', () {
    ...
    var mrz = MRZ(Uint8List.fromList(
        "I<UTOSTEVENSON<<PETER<JOHN<<<<<<<<<<D23145890<UTO3407127M95071227349<<<8"
            .codeUnits));
    expect(DBAKey.fromMRZ(mrz).keySeed,
        "b366ad857ddca2b08c0e299811714730".parseHex());

    mrz = MRZ(Uint8List.fromList(
        "I<UTOERIKSSON<<ANNA<MARIA<<<<<<<<<<<L898902C<3UTO6908061F9406236<<<<<<<2"
            .codeUnits)); // Note: composite CD changed from 8 to 2
    expect(
        DBAKey(mrz.documentNumber, mrz.dateOfBirth, mrz.dateOfExpiry).keySeed,
    ...
    mrz = MRZ(Uint8List.fromList(
        "I<UTOL898902C<3<<<<<<<<<<<<<<<6908061F9406236UTO<<<<<<<<<<<2ERIKSSON<<ANNA<MARIA<<<<<<<<<<"
            .codeUnits)); // Note: Composite CD changed from 1 to 2
    expect(DBAKey.fromMRZ(mrz).keySeed,
        "239AB9CB282DAF66231DC5A4DF6BFBAE".parseHex());
  });
  ...
  test('BAC session establishment test', () {
    ...
    final tvMRZ = MRZ(Uint8List.fromList(
        "I<UTOL898902C<3<<<<<<<<<<<<<<<6908061F9406236UTO<<<<<<<<<<<2ERIKSSON<<ANNA<MARIA<<<<<<<<<<"
            .codeUnits)); // Note: Composite CD changed from 1 to 2
    ...
    expect(
        BAC.generateS(RNDicc: tvRNDicc, RNDifd: tvRNDifd, Kifd: tvKifd), tvS);
    ...
    expect(
        BAC.calculateSCC(RNDifd: tvRNDifd, RNDicc: tvRNDicc).toBytes(), tvSCC);
  });
}
