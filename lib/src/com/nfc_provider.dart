...
class NfcProvider extends ComProvider {
  ...
  Future<Uint8List> transceive(final Uint8List data,
    ...
    try {
      _log.finest("Transceive: ${safeHex(data)}");
      final response = await FlutterNfcKit.transceive(data,
          timeout: timeout ?? this.timeout);
      _log.finest("Response: ${safeHex(response)}");
      return response;
    } on Exception catch (e) {
      _log.warning("Transceive error: $e");
      throw NfcProviderError.fromException(e);
    }
  }
}
