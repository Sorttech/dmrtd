...
class DHPace {
  ...
  void generateKeyPairFromPriv({required Uint8List privKey}) {
    _log.fine(
        "DHPace.generateKeyPairFromPriv; Generating key pair for domain parameter ${selectedDomainParameter.name}.");
    _log.sdVerbose(
        "DHPace.generateKeyPairFromPriv; PrivateKey: ${privKey.map((byte) => byte.toRadixString(16).padLeft(2, '0')).join()}");
  ...
  void generateKeyPairWithCustomGenerator(
      {required BigInt ephemeralGenerator, int? seed = null}) {
    _log.fine(
    ...
    _engineEphemeral =
        DHpkcs3Engine(parameterSpec: domainParametersEphemeral, seed: seed);
  ...
  void setEphemeralKeyPair(
      {required Uint8List private, required BigInt ephemeralGenerator}) {
    _log.fine(
  ...
  BigInt getSharedSecret({required Uint8List otherPubKey}) {
    ...
    return _engine!
        .computeSecretKey(otherPublicKey: Utils.uint8ListToBigInt(otherPubKey));
  }
  ...
  Uint8List getMappedGenerator(
      {required Uint8List otherPubKey, required Uint8List nonce}) {
    _log.fine(
...
class DHPaceCurve0 extends DHPace {
  DHPaceCurve0() : super(id: 0, domainParameters: RFC5114_1024_MODP_160);
}

class DHPaceCurve1 extends DHPace {
  DHPaceCurve1() : super(id: 1, domainParameters: RFC5114_2048_MODP_224);
}

class DHPaceCurve2 extends DHPace {
  DHPaceCurve2() : super(id: 2, domainParameters: RFC5114_2048_MODP_256);
}
