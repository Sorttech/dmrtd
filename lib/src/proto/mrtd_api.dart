...
class MrtdApi {
  ...
  Future<Uint8List> readFileBySFI(int sfi) async {
    ...
    try {
      ...
      // If SFI read fails, try selecting by FID if it's a known SFI
      if (e.sw == StatusWord.referencedDataNotFound ||
          e.sw == StatusWord.notSupported) {
        _log.info(
            "SFI read failed (sw=${e.sw}), trying to select and read by FID...");
        int? fid;
        // Map common SFIs to FIDs
